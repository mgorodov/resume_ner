...
### Почему Entity F1, а не Token Accuracy?

В проекте используются **две метрики**, но **основная — Entity-level F1 (strict match)**:

| Метрика | Что измеряет | Значение для production |
|---------|--------------|------------------------|
| **Token Accuracy** | Доля токенов с правильно предсказанным标签ом (B-PER, I-PER, O, etc.) | ⚠️ **Вводит в заблуждение** |
| **Entity F1 (strict)** | Доля правильно распознанных сущностей (точное совпадение границ + типа) | ✅ **Основная метрика** |

### Token Accuracy — почему не подходит?

```
Пример резюме (200 токенов):
- 180 токенов — "O" (не сущности)
- 20 токенов — сущности (Name, Skills, Companies, etc.)

Модель предсказала всё как "O":
- Token Accuracy = 180/200 = 90% (выглядит отлично!)
- Entity F1 = 0% (не нашла ни одной сущности!)
```

**Проблема:** Сильный дисбаланс классов — большинство токенов это "O" (outside entity). Модель может получить высокую accuracy, просто предсказывая всегда "O".

### Entity F1 (strict match) — почему основная?

```python
# Сущность считается правильно распознанной, только если:
# 1. Точно совпадает начало (start_char)
# 2. Точно совпадает конец (end_char)
# 3. Точно совпадает тип (label)

True:  [(0, 10, "Name"), (50, 60, "Skills")]
Pred:  [(0, 10, "Name"), (50, 65, "Skills")]  # ← границы не совпали!

Result: TP=1, FP=1, FN=1
Precision = 1/2 = 0.5
Recall = 1/2 = 0.5
...
**Преимущества:**
- ✅ Требует **точного попадания в границы** — как в production
- ✅ Учитывает **тип сущности** — нельзя перепутать "Skills" и "Companies"
- ✅ **Не прощает** почти правильные ответы ("Infosys" vs "Infosys Limited")

### Формулы

```python
# Token-level метрики
Token Accuracy = (правильные токены) / (все токены)

# Entity-level метрики (strict match)
True Positive (TP)  = сущность найдена точно (start, end, label)
False Positive (FP) = модель нашла лишнюю сущность
False Negative (FN) = модель пропустила сущность

Precision = TP / (TP + FP)  # сколько из найденного верно
Recall    = TP / (TP + FN)  # сколько из истины найдено
...
В Чекпоинте 7 модель эволюционирует от простого ML-эксперимента до **production-ready решения** с полным трекингом экспериментов, версионированием артефактов и воспроизводимостью обучения.

**Лучшая модель (B+aug)** достигает **Entity-F1 = 0.6040** и регистрируется в MLflow Model Registry с тегом `stage=PRD`.

**Ключевая метрика:** Entity-level F1 (strict match) — требует точного совпадения границ и типа сущности, что критично для production-систем извлечения информации из резюме.
